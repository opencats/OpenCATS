...
# Check if the cats_test table is available before running Behat
if ! mysql --host=opencatsdb --user=dev --password=dev -e "USE cats_test; SHOW TABLES';"; then
    echo "ERROR: Database cats_test or the required table is not available."
    exit 1
fi
