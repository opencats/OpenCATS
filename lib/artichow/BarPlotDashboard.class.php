...
class awBarPlotDashboard extends awPlot implements awLegendable
{
    ...
    public function drawComponent(awDrawer $drawer, $x1, $y1, $x2, $y2, $aliasing)
    {
        ...
        if ($this->number !== 0) {
            ...
            // Handle the case where $this->number is zero
            // For example, you can log an error, set a default value, or throw an exception
            error_log('Error: Division by zero in bar size calculation. Setting bar size to default value.');
            $barSize = 1; // or any appropriate default value
        }
        ...
        foreach ($datayReal as $key => $value) {
            if ($value !== null) {
                ...
                $position2 = awAxis::toPosition(
                    $this->xAxis,
                    $this->yAxis,
                    new awPoint($key, $value ? $this->datay[$key] / 2 : 0)
                );
