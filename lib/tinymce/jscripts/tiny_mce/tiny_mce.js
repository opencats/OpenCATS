...
;TinyMCE_Engine.prototype = {
    init: function(settings) {
        ...
        if (!tinyMCE.baseURL) {
            ...
            if (head) {
                for (i = 0,
                nl = head.getElementsByTagName('script'); i < nl.length; i++)
                    elements.push(nl[i])
            }
            for (i = 0,
            nl = document.getElementsByTagName('script'); i < nl.length; i++)
                elements.push(nl[i]);
        ...
        this.uniqueURL = 'javascript:void(091039730);';
    ...
    loadCSS: function(url) {
        ...
        var x = 0, i = 0, nl, le;
        for (x = 0,
        csslen = ar.length; x < csslen; x++) {
            if (ar[x] != null && ar[x] != 'null' && ar[x].length > 0) {
                for (i = 0,
                lflen = this.loadedFiles.length; i < lflen; i++) {
    ...
    importCSS: function(doc, css) {
        ...
        for (x = 0,
        csslen = css_ary.length; x < csslen; x++) {
            css_file = css_ary[x];
    ...
    if (!tinyMCE.hasTheme(t)) {
        ...
        ;TinyMCE_Control.prototype = {
            ...
            _fixTrailingNbsp: function() {
                ...
                if (e && tinyMCE.blockRegExp.test(e.nodeName) && e.firstChild) {
                    ...
                    if (v && v.length > 1 && /(^\u00a0|\u00a0$)/.test(v)) {
                        ...
                        s.selectNode(e.firstChild, true, false, false);
                    }
                }
            },
            ...
            execCommand: function(command, user_interface, value) {
                ...
                case "JustifyFull":
                case "JustifyRight":
                    var el = tinyMCE.getParentNode(focusElm, function(n) {
                        return tinyMCE.getAttrib(n, 'align')
                    });
                    if (el) {
                ...
                case "mceSetAttribute":
                    ...
                    break;
                case "mceSetCSSClass":
                    this.execCommand("mceSetStyleInfo", false, {
                        command: "setattrib",
                        name: "class",
                        value: value
                    });
        ...
        tinyMCE.add(TinyMCE_Engine, {
            ...
            tinyMCE.add(TinyMCE_Engine, {
                ...
                selectElements: function(n, na, f) {
                    var i, a = [], nl, x;
                    for (x = 0,
                    na = na.split(','); x < na.length; x++)
                        for (i = 0,
                        nl = n.getElementsByTagName(na[x]); i < nl.length; i++)
            ...
            tinyMCE.add(TinyMCE_Engine, {
                ...
                convertAbsoluteURLToRelativeURL: function(base_url, url_to_relative) {
                    var baseURL = this.parseURL(base_url)
                      , targetURL = this.parseURL(url_to_relative);
            ...
            ;TinyMCE_Selection.prototype = {
                ...
                getAbsPosition: function(n) {
                    var pos = tinyMCE.getAbsPosition(n)
                      , ipos = tinyMCE.getAbsPosition(this.instance.iframeElement);
                    return {
            ...
            var TinyMCE_ForceParagraphs = {
                _insertPara: function(inst, e) {
                    ...
                    startNode = startNode.nodeName == "HTML" ? doc.body : startNode;
                    ...
                    do {
                        if (node == body || node.nodeType == 9 || tinyMCE.isBlockElement(node))
                            break;
                        startChop = node
                    } while ((node = node.previousSibling ? node.previousSibling : node.parentNode));node = endChop;
                    do {
                        if (node == body || node.nodeType == 9 || tinyMCE.isBlockElement(node))
                            break;
                        endChop = node
                    } while ((node = node.nextSibling ? node.nextSibling : node.parentNode));if (startChop.nodeName == "TD")
            ...
            tinyMCE.add(TinyMCE_Engine, {
                debug: function() {
                    ...
                    for (i = 0,
                    a = this.debug.arguments; i < a.length; i++) {
                        m += a[i];
                        if (i < a.length - 1)
                            m += ', '
                    }
