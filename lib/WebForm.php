...
class WebForm
{
    ...
    public function addField(
        ...
        switch ($type) {
            case WFT_CURRENCY:
                if ($regex_test == '') {
                    $regex_test = WTR_CURRENCY;
                }
                if ($regex_fail == '') {
                    $regex_fail = WTRF_CURRENCY;
                }
                $minlen = 0;
                $maxlen = 10;
                break;
            case WFT_PHONE:
                if ($regex_test == '') {
                    $regex_test = WTR_PHONE;
                }
                if ($regex_fail == '') {
                    $regex_fail = WTRF_PHONE;
                }
                $minlen = 10;
                $maxlen = 13;
                break;
            case WFT_EMAIL:
                if ($regex_test == '') {
                    $regex_test = WTR_EMAIL;
                }
                if ($regex_fail == '') {
                    $regex_fail = WTRF_EMAIL;
                }
                $minlen = 10;
                $maxlen = 255;
                break;
            case WFT_CC_NUMBER:
                if ($regex_test == '') {
                    $regex_test = WTR_CC_NUMBER;
                }
                if ($regex_fail == '') {
                    $regex_fail = WTRF_CC_NUMBER;
                }
                $size = 30;
                $minlen = 15;
                $maxlen = 19;
            ...
            case WFT_CC_TYPE:
                $defaultValue = [
                    [
                        'id' => WF_CARD_NAME_VISA,
                        'caption' => WF_CARD_NAME_VISA,
                        'selected' => false,
                    ],
                    [
                        'id' => WF_CARD_NAME_MASTERCARD,
                        'caption' => WF_CARD_NAME_MASTERCARD,
                        'selected' => false,
                    ],
                    [
                        'id' => WF_CARD_NAME_AMERICANEXPRESS,
                        'caption' => WF_CARD_NAME_AMERICANEXPRESS,
                        'selected' => false,
                    ],
                    [
                        'id' => WF_CARD_NAME_DISCOVER,
                        'caption' => WF_CARD_NAME_DISCOVER,
                        'selected' => false,
                    ],
                ];
                break;
                ...
                if ($field['required'] == false && ! strlen($cardNumber)) {
                    ...
                    if ($cardType == -1) {
                        $errors[] = 'The credit card number you entered is not a recognized Visa, MasterCard, American Express '
                            . 'or Discover card.';
                    } elseif (! $this->isCardNumberValid($cardType, $cardNumber)) {
                    ...
                    if (strlen($value) < $field['length'][0] || strlen($value) > $field['length'][1]) {
                        if ($field['length'][0] == $field['length'][1]) {
                            if (strlen(trim($field['caption'])) > 0) {
                                ...
                                $errors[] = sprintf(
                                    "This field must be %d characters in length",
                                    $field['length'][0]
                                );
                            }
                        } else {
                            $errors[] = sprintf(
                                "%s must be between %s characters in length",
                                $field['caption'],
                                implode(' and ', $field['length'])
                            );
                        }
                    }
                }
