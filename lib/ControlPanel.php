...
class ControlPanel
{
    ...
    public function getWebForm($addRecord = false)
    {
        ...
        foreach ($_GET as $name => $value) {
            if (! strcmp($name, 'cpPageState')) {
                $html .= sprintf(
                    '<input type="hidden" name="cpPageState" value="%d" />',
                    CPPS_LISTVIEW
                );
            } elseif (! strcmp($name, 'a') || ! strcmp($name, 'm') || ! strcmp($name, 'siteID')) {
                $html .= sprintf(
        ...
        } elseif (! $addRecord && $this->_permissions & CPP_EDIT) {
        ...
        foreach ($this->_tables as $tableName => $tableData) {
            foreach ($tableData['fields'] as $fieldName => $fieldData) {
                ...
                $this->_wf->addField(
                    $fieldData['webFormParams']['name'],
                    $fieldData['webFormParams']['caption'],
                    $fieldData['webFormParams']['type'],
                    $fieldData['webFormParams']['required'],
                    $fieldData['webFormParams']['size'],
                    $fieldData['webFormParams']['minlen'],
                    $fieldData['webFormParams']['maxlen'],
                    $fieldData['webFormParams']['defaultValue'],
                    $fieldData['webFormParams']['regex_test'],
                    $fieldData['webFormParams']['regex_fail'],
                    $fieldData['webFormParams']['helpBody'],
                            ...
                            if (! strcmp($subFieldData['uniqueID'], $fieldName)) {
                                if ($addRecord) {
                                    ...
                                    if ($this->isFieldChange($row[$fieldName], $fieldValue, $subFieldData)) {
                                        ...
                                        $updateSql[$subTableName] .= sprintf(
                                            '%s.%s = %s',
                                            $subTableName,
                                            $subFieldName,
                                            $this->getFieldDBText($subFieldData, $fieldValue)
                                        );
                                    }
                                }
                            }
    ...
    public function getListView()
    {
        ...
        while ($row = mysqli_fetch_array($rs, MYSQLI_ASSOC)) {
            ...
            $highlightJSFlat = sprintf(
                'onmouseover="%s" onmouseout="%s"',
                $highlightJS['onmouseover'],
                $highlightJS['onmouseout']
            );
            ...
            foreach ($this->_tables as $tableName => $tableData) {
                ...
                foreach ($tableData['fields'] as $fieldName => $fieldData) {
                    ...
                    if (isset($fieldData['activeField']) && $fieldData['activeField'] == true &&
                        isset($fieldData['section']) && is_array($fieldData['section']) &&
                        in_array(CP_LISTVIEW, $fieldData['section'])) {
                        if (! isset($headerRow[$fieldData['uniqueID']])) {
                            $headerRow[$fieldData['uniqueID']] = '';
                        }
                        if (! isset($myRow[$fieldData['uniqueID']])) {
                            $myRow[$fieldData['uniqueID']] = '';
                        }
                        ...
                        $myRow[$fieldData['uniqueID']] .= sprintf(
                            '<td %s>%s</td>',
                            $td_final,
                            $td_html
                        );
            ...
            if ($this->_permissions & CPP_EDIT || $this->_permissions & CPP_DELETE) {
                ...
                $myRow['admin.row'] .= sprintf(
                    '<td %s id="cp_adminDOTrow%d" class="%s" align="right" valign="center">',
                    $highlightJSFlat,
                    $rowNum,
                ...
                if ($this->_permissions & CPP_DELETE && $this->_deleteBoundriesTable != '') {
                    ...
                    $myRow['admin.row'] .= sprintf(
                        '<td style="padding-right: 1px;"><img '
        ...
        if ($this->_showCurrencySums && $rsCount > 0 && $currencySql != '') {
            $infoHtml .= sprintf(
                '<tr><td colspan="%d" valign="center" align="center" '
                ...
                foreach ($tableData['fields'] as $fieldName => $fieldData) {
                    ...
                    if (isset($fieldData['activeField']) && $fieldData['activeField'] == true &&
                        ...
                        if ($fieldData['webFormType'] == WFT_CURRENCY && isset($currencySums[$fieldData['uniqueID']])) {
                            $myRow[$fieldData['uniqueID']] = sprintf(
                                '<td align="right" valign="center" class="cpFieldSum">$%s</td>',
        ...
        if ($numPages > 1) {
            ...
            if ($pager_CurrentPage > 0) {
                $pagerHtml .= sprintf(
                    '<a href="%s&cp_CurrentPage=%d">Previous Page</a>',
                    $url,
                    $pager_CurrentPage
                );
            }
            if ($pager_CurrentPage < $numPages - 1) {
                ...
                $pagerHtml .= sprintf(
                    '<a href="%s&cp_CurrentPage=%d">Next Page</a>',
                    $url,
                    $pager_CurrentPage + 2
                );
            }
        ...
        foreach ($this->_tables as $tableName => $tableData) {
            foreach ($tableData['fields'] as $fieldName => $fieldData) {
                if (! strcmp($fieldName, $name) || (isset($fieldData['uniqueID']) && ! strcmp($fieldData['uniqueID'], $name))) {
                    ...
                    $this->_tables[$tableName]['fields'][$fieldName]['webFormParams'] = [
                        'name' => $name,
                        'caption' => $caption,
                        'type' => $type,
                        'required' => $required,
    ...
    public function getTablesSQL($relationshipSql = '', $limitSql = '', $selectSql = '')
    {
        ...
        foreach ($this->_tables as $tableName => $tableData) {
            ...
            foreach ($this->_tables as $subTableName => $subTableData) {
                if (! strcmp($subTableName, $tableName)) {
                    continue;
