...
switch ($action)
{
    ...
    case 'databaseConnectivity':
        ...
        if (isset($_REQUEST['user']))
        {
            ...
            {
                osatutil::changeConfigSetting('DATABASE_USER', "'" . $_REQUEST['user'] . "'");
            }

            if (isset($_REQUEST['pass']))
            {
                osatutil::changeConfigSetting('DATABASE_PASS', "'" . $_REQUEST['pass'] . "'");
            }

            if (isset($_REQUEST['host']) && !empty($_REQUEST['host']))
            {
                osatutil::changeConfigSetting('DATABASE_HOST', "'" . $_REQUEST['host'] . "'");
            }

            if (isset($_REQUEST['name']) && !empty($_REQUEST['name']))
            {
                osatutil::changeConfigSetting('DATABASE_NAME', "'" . $_REQUEST['name'] . "'");
            }
    ...
    case 'setMailSettings':
        ...
        else
        {
            ...
            if ($mailSupport == 4)
            {
                osatutil::changeConfigSetting('MAIL_MAILER', '3');
                osatutil::changeConfigSetting('MAIL_SMTP_AUTH', 'true');
            }
            else
            {
                osatutil::changeConfigSetting('MAIL_MAILER', sprintf('%d', $mailSupport));
                osatutil::changeConfigSetting('MAIL_SMTP_AUTH', 'false');
            }

            osatutil::changeConfigSetting('MAIL_SENDMAIL_PATH', '"' . $mailSendmailPath . '"');
            osatutil::changeConfigSetting('MAIL_SMTP_HOST', '"' . $mailSmtpHost . '"');
            osatutil::changeConfigSetting('MAIL_SMTP_PORT', sprintf('%d', $mailSmtpPort));
    ...
    case 'testResumeParsing':
        ...
        osatutil::changeConfigSetting('ANTIWORD_PATH', '"' . $antiwordWithSlashes . '"');

        $pdftotextPath = $_REQUEST['pdfExecutable'];
        $pdftotextWithSlashes = str_replace('\\', '\\\\', $pdftotextPath);
        osatutil::changeConfigSetting('PDFTOTEXT_PATH', '"' . $pdftotextWithSlashes . '"');

        $html2textPath = $_REQUEST['htmlExecutable'];
        $html2textWithSlashes = str_replace('\\', '\\\\', $html2textPath);
        osatutil::changeConfigSetting('HTML2TEXT_PATH', '"' . $html2textWithSlashes . '"');

        $unrtfPath = $_REQUEST['rtfExecutable'];
        $unrtfWithSlashes = str_replace('\\', '\\\\', $unrtfPath);
        osatutil::changeConfigSetting('UNRTF_PATH', '"' . $unrtfWithSlashes . '"');
    ...
    case 'setupOptional':
        ...
        osatutil::changeConfigSetting('OFFSET_GMT', ($timeZone));
