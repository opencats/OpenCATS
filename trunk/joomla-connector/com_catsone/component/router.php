...
function CatsoneBuildRoute(&$query)
{
	static $items;
	...
	if (!$items) {
		$component	= &JComponentHelper::getComponent('com_catsone');
		$menu		= &JSite::getMenu();
		$items		= $menu->getItems('componentid', $component->id);
	}

	// Break up the Catsone id into numeric and alias values.
	...
	if (is_array($items))
	{
		...
		if (!$itemid)
		{
			foreach ($items as $item)
			{
				...
				if (isset($item->query['view']) && $item->query['view'] == 'Catsone'
					&& isset($query['view']) && $query['view'] == 'Catsone'
		...
		if (!$itemid)
		{
			foreach ($items as $item)
			{
				if (isset($query['view']) && $query['view'] == 'Catsone'
					&& isset($item->query['view']) && $item->query['view'] == 'category')
				{
					// Check for an undealt with Catsone id.
					if (isset($query['id']))
					{
						// This menu item links to the Catsone view but we need to append the Catsone id to it.
						$itemid		= $item->id;
...
function CatsoneParseRoute($segments)
{
	$vars	= array();
	...
	if (is_object($item))
	{
		...
		if (isset($item->query['view']) && $item->query['view'] == 'Catsone' && isset($segments[0]))
		{
			// Break up the Catsone id into numeric and alias values.
