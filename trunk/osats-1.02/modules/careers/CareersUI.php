...
class CareersUI extends UserInterface
{
        ...
        */
        ...
        else if ($p == 'onApplyToJobOrder')
        {
            ...
            if ((isset($_GET[$id='questionnairePostBack']) && $_GET[$id] == '1') || !$questionnaireID)
            {
                ...
                $template['Content'] = str_replace('<title>', $jobOrderData['title'], $template['Content']);
                $template['Content'] = str_replace('<a-jobDetails>', '<a href="' . osatutil::getIndexName() . '?m=careers'.(isset($_GET['templateName']) ? '&templateName='.urlencode($_GET['templateName']) : '').'&p=showJob&ID='.$_POST['ID'].'">', $template['Content']);
            }
            else
            {
                ob_start();
                ...
                $formData = '<form name="postQuestionnaire" id="postQuestionnaire" '
                    ...
                    . osatutil::getIndexName() . '?m=careers&p=onApplyToJobOrder'
        ...
        else if ($p == 'showJob')
        {
            ...
            {
                $template['Content'] = str_replace('<a-applyToJob', '<a href="'.osatutil::getIndexName().'?m=careers'.(isset($_GET['templateName']) ? '&templateName='.urlencode($_GET['templateName']) : '').'&p=candidateRegistration&ID='.$jobID.'"', $template['Content']);
            }
            else
            {
                $template['Content'] = str_replace('<a-applyToJob', '<a href="'.osatutil::getIndexName().'?m=careers'.(isset($_GET['templateName']) ? '&templateName='.urlencode($_GET['templateName']) : '').'&p=applyToJob&ID='.$jobID.'"', $template['Content']);
            }
    ...
    private function getResultsTable($rs, $settings, $unformatted = false, $parameters = '')
    {
        ...
        foreach ($rs as $index => $line)
        {
            ...
            $html .= '<td>';
            $html .= '<a href="' . osatutil::getIndexName() . '?m=careers' . (isset($_GET['templateName']) ? '&amp;templateName=' . urlencode($_GET['templateName']) : '').'&amp;p=showJob&amp;ID=' . $line['jobOrderID'] . '">';
