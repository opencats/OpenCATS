        ...
        <div id="contents"<?php echo !$this->totalJobOrders ? ' style="background-color: #E6EEFF; padding: 0;"' : ''; ?>>
            ...
            <table width="100%">
                <tr>
                    ...
                    <td><h2><?php echo __('Job Orders').': '.__('Home')?></h2></td>
                                ...
                                <tr>
                                    <td>
                                        <select name="view" id="view" onchange="<?php echo($this->dataGrid->getJSAddFilter('Status', '==', 'this.value', 'true')); ?>" class="selectBox">
                                            <option value="Active / OnHold / Full"<?php if ($this->dataGrid->getFilterValue('Status') == 'Active / OnHold / Full'): ?> selected="selected"<?php endif; ?>><?php _e('Active') ?> / <?php _e('On Hold') ?> / <?php _e('Full') ?></option>
                                            <option value="Active"<?php if ($this->dataGrid->getFilterValue('Status') == 'Active'): ?> selected="selected"<?php endif; ?>><?php _e('Active') ?></option>
                                            <option value="OnHold / Full"<?php if ($this->dataGrid->getFilterValue('Status') == 'OnHold / Full'): ?> selected="selected"<?php endif; ?>><?php _e('On Hold') ?> / <?php _e('Full') ?></option>
                                            <option value="Closed / Canceled"<?php if ($this->dataGrid->getFilterValue('Status') == 'Closed / Canceled'): ?> selected="selected"<?php endif; ?>><?php _e('Closed') ?> / <?php _e('Canceled') ?></option>
                                            <option value="Upcoming / Lead"<?php if ($this->dataGrid->getFilterValue('Status') == 'Upcoming / Lead'): ?> selected="selected"<?php endif; ?>><?php _e('Upcoming') ?> / <?php _e('Lead') ?></option>
                                            <option value=""<?php if ($this->dataGrid->getFilterValue('Status') == ''): ?> selected="selected"<?php endif; ?>><?php _e('All') ?></option>
                                        </select>
                                    </td>
                                    ...
                                    <td valign="top" align="right" nowrap="nowrap">
                                        ...
                                        <label for="onlyMyJobOrders"><?php _e('Only My Job Orders') ?></label>&nbsp;
                                    ...
                                    <td valign="top" align="right" nowrap="nowrap">
                                        ...
                                        <label for="onlyHotJobOrders"><?php _e('Only Hot Job Orders') ?></label>&nbsp;
                                    </td>
                                </tr>
                            </table>
                        </form>
            ...
            <p class="note">
                <span style="float:left;"><?php
                  echo __('Job Orders').' - '.__('Page').' '.$this->dataGrid->getCurrentPageHTML();
                  echo ' ('; 
                  echo format_number_choice('countItems', array($this->dataGrid->getNumberOfRows()), $this->dataGrid->getNumberOfRows());
                  echo ') (';
                  if ($this->dataGrid->getFilterValue('Status') != '')
                    echo ($this->dataGrid->getFilterValue('Status'));
                  else
                    echo __('All');
                  ...
                  if ($this->dataGrid->getFilterValue('OwnerID') ==  $this->userID)
                    echo ' ('.__('Only My Job Orders').')';

                  if ($this->dataGrid->getFilterValue('IsHot') == '1')
                    echo ' ('.__('Only Hot Job Orders').')';
                ?></span>
            ...
            <table cellpadding="0" cellspacing="0" border="0" width="956">
                ...
                <td style="padding-left: 62px;" align="center" valign="center">
                    ...
                    <?php _e('Add a job order, then attach candidates to the pipeline with their status interviewing, qualifying, etc.') ?>
                    </div>

                    <a href="javascript:void(0);"  onclick="showPopWin('<?php echo osatutil::getIndexName(); ?>?m=joborders&amp;a=addJobOrderPopup', 400, 250, null);">
