...
class ImportUI extends UserInterface
{
    ...
    */
    ...
    public function onImportFieldsDelimited()
    {
        ...
        {
            $errorHtml = '<span class="bold">'.__('First') . self::MAX_ERRORS . ' '.__('Errors'). ' ('.__('of') . ($totalRows - $totalImported) . '):</span><br /><br />' . $errorHtml;
        }
        ...
        $message .= osatutil::getIndexName() . '?m=import&amp;a=revert&amp;importID=' . $importID . '\';" value="' .__('Revert Import'). '" class="button">';

        if ($totalRows != $totalImported)
        {
            $message .= '<input type="button" onclick="document.location.href=\'';
            $message .= osatutil::getIndexName() . '?m=import&amp;a=viewerrors&amp;importID=' . $importID . '\';" value="' . __('View Import Errors' ). ' class="button">';
        }
    ...
    */
    private function addToCandidates($dataFields, $dataNamed, $dataForeign, $importID)
    {
        ...
        {
            return __('Required fields are missing.');
        }
    ...
    */
    private function addToContacts($dataFields, $dataNamed, $dataForeign, $importID)
    {
        ...
        if (!isset($dataNamed['first_name']) && !isset($dataNamed['last_name']))
        {
            ...
            {
                $error = __('Required fields are missing.');
                if ($genCompany)
                {
    ...
    */
        ...
        else if ($step == 4)
        {
                ...
                !count($importedDuplicates))
            {
                $this->_template->assign('errorMessage', '<b style="font-size: 20px;">Information no Longer '
                    ...
                    . 'mass resume import, <a style="font-size: 16px;" href="' . osatutil::getIndexName() . '?m=import&a=massImport&'
                    . 'step=1">click here</a>.'
                );
            }
    ...
    private function importBulkResumes()
    {
        ...
        osatutil::transferRelativeURI('m=import&a=massImport&step=2');
    }
}
