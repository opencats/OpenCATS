...
?>
    <div id="main">
        ...
        <div id="contents">
                ...
                <table class="searchTable" id="importHide1">
                    <tr>
                        <td>
                            ...
                            <input type="button" onclick="document.location.href='<?php echo(osatutil::getIndexName()); ?>?m=import&amp;a=viewpending';" value="<?php _e('View Recent Imports')?>" class="button" />
                        </td>
                    </tr>
                </table>
            ...
            <?php else: ?>
                <p class="note" id="importHide0"><?php _e('Warning')?>!</p>
            ...
            <p class="warning" id="importShow0" style="display:none;"><?php _e('Please Wait')?>!</p>

            <table class="searchTable" id="importShow1" style="display:none;">
                <tr>
                    <td>
                        <?php _e('Please wait')?>! <?php _e('Importing data may take a few minutes.')?><br />
                        <img src="images/loading.gif" />
                    </td>
                </tr>
            </table>

            <p class="note" id="importHide2"><?php _e('Import Data')?></p>

            <form name="importDataForm" id="importDataForm" action="<?php echo(osatutil::getIndexName()); ?>?m=import&amp;a=import&amp;#step2" enctype="multipart/form-data" method="post" autocomplete="off" <?php if (isset($this->contactsUploadNotice) && $this->contactsUploadNotice): ?> onsubmit="return checkField(<?php echo(count($this->theFields)); ?>, 'company_id', __('You must have 1 field set as Company.')) && showLoading();"<?php else: ?> onsubmit="return showLoading();"<?php endif; ?>>
                ...
                <table class="searchTable" width="740" id="importHide3">
                    ...
                    <tr>
                        <td class="tdVertical">
                            <label id="dataContainingDisabledLabel" for="dataContainingDisabled"><?php _e('File Format')?>:</label>
                        </td>
                        <td class="tdData">
                            <?php if ($this->dataContaining == 'tab'): ?>
                                <?php _e('Tab Delimited Data')?>
                            <?php elseif ($this->dataContaining == 'csv'): ?>
                                <?php _e('Comma Delimited Data')?> (CSV)
                            <?php endif; ?>
                    ...
                    <tr>
                        <td class="tdVertical">
                            <label id="importIntoLabel" for="importIntoDisabled"><?php _e('Import Into')?>:</label>
                        </td>
                        <td class="tdData">
                            <?php if ($this->importInto == __('Candidates')): ?>
                                <img src="images/candidate_inline.gif">&nbsp;<?php _e('Candidates')?>
                            <?php elseif ($this->importInto == __('Companies')): ?>
                                <img src="images/mru/company.gif">&nbsp;<?php _e('Companies')?>
                            <?php elseif ($this->importInto == __('Contacts')): ?>
                    ...
                    <input type="reset"  class="button" name="reset" id="importHide4" value="<?php _e('Reset')?>" onclick="document.location.href='<?php echo(osatutil::getIndexName()); ?>?m=import&amp;a=import';" />&nbsp;

            <br />
            <br />


            <p class="note" id="importHide5"><a name="step2"><?php _e('Map Data')?></a></p>
            ...
            <table class="searchTable" id="importHide9">
                <tr>
                    <td><?php _e('The following fields were detected in your data. Please pick where to put the data from each field, then press import at the bottom.')?></td>
                </tr>
            </table>
                    ...
                    <tr>
                        ...
                        <td class="tdData">
                            ...
                            <span <?php if (!$match): ?>style="display:none;"<?php endif; ?> id="importIntoSpan<?php echo($fieldIndex); ?>">
                                &nbsp;<?php echo($this->importInto); ?> <?php _e('Field')?>:&nbsp;
                            ...
                            <div id="importSample<?php echo($fieldIndex); ?>" style="display:none;">
                                ...
                                <table class="searchTable">
                                    <tr>
                                        <td>
                                            <span class="bold"><?php _e('Sample Data')?>:</span>
            ...
            <input type="reset"  class="button" name="reset"  id="importHide8"  value="<?php _e('Reset')?>" onclick="for (var i = 0; i < <?php echo(count($this->theFields)); ?>; i++) evaluateFieldSelection(i); " />&nbsp;
