...
<?php TemplateUtility::printModalHeader('Candidates', array('modules/candidates/validator.js'), 'Create Candidate Attachment'); ?>
    ...
    <?php else: ?>
        <?php if(isset($this->resumeText) && $this->resumeText == ''): ?>
            <p>The file has been successfully attached, but OSATS was unable to index the resume keywords to make the document searchable.  The file format may be unsupported by CATS.</p>
        <?php else: ?>
