...
if [ "${TRAVIS_TAG}" != "false" ]; then
	...
	zip -q -x INSTALL_BLOCK -x docker/data -x=docker/persist -x attachments -x code/tests/_output -x code/var/cache -x code/var/logs -x code/var/sessions -x code/src/AppBundle/tests/_output -x code/src/AppBundle/tests/_support/_generated -r /tmp/opencats-$TRAVIS_TAG-full.zip $TRAVIS_BUILD_DIR
