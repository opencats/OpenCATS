...
<?php TemplateUtility::printModalHeader(__('Candidates'), array('modules/candidates/validator.js'), __('Create Candidate Attachment')); ?>
    ...
    <?php if (!$this->isFinishedMode){ ?>
        <form name="createAttachmentForm" id="createAttachmentForm" action="<?php echo(CATSUtility::getIndexName()); ?>?m=candidates&amp;a=createAttachment" enctype="multipart/form-data" method="post" onsubmit="return checkCreateAttachmentForm(document.createAttachmentForm);">
            ...
            <table class="editTable">
                ...
                <tr>
                    <td class="tdVertical"><?php echo __("Resume");?>:</td>
                    <td>
    ...
    <?php } else { ?>
        <?php if(isset($this->resumeText) && $this->resumeText == ''): ?>
            <p><?php echo __("The file has been successfully attached, but CATS was unable to index the resume keywords to make the document searchable.  The file format may be unsupported by CATS.");?></p>
        <?php else: ?>
